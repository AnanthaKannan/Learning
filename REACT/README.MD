...
1.  Reset State Manually
2.  Use an Initial State Variable
3.  Using a Reducer (for Complex State)
4.  Reset State by Changing the Component’s key
```js
function App() {
  ...
  return (
    <div>
      ...
      <button onClick={() => setResetKey(prev => prev + 1)}>Reset Counter</button>
    </div>
  );
}
